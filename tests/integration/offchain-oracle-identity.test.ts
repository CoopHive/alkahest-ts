...
test("contextless offchain identity oracle flow", async () => {
  ...
  const goodDecision = await testContext.charlie.client.arbiters.waitForTrustedOracleArbitration(
    goodFulfillment.uid,
  ...
  const badDecision = await testContext.charlie.client.arbiters.waitForTrustedOracleArbitration(
    badFulfillment.uid,
    testContext.charlie.address,
  );
