...
test("encodeTrustedPartyDemand", () => {
  ...
  // Decode to verify
  const decodedDemand = clientOracle.arbiters.decodeTrustedPartyDemand(encodedDemand);
...
test("encodeSpecificAttestationDemand", () => {
  const uid = "0x1111222233334444555566667777888899990000aaaabbbbccccddddeeeeffff";
  const demand = {
    uid: uid as `0x${string}`,
  };
  
  // Encode demand data
  const encodedDemand = clientOracle.arbiters.encodeSpecificAttestationDemand(demand);
  expect(encodedDemand).toBeString();
  expect(encodedDemand.startsWith("0x")).toBe(true);
  
  // Decode to verify
  const decodedDemand = clientOracle.arbiters.decodeSpecificAttestationDemand(encodedDemand);
...
test("encodeTrustedOracleDemand", () => {
  ...
  // Decode to verify
  const decodedDemand = clientOracle.arbiters.decodeTrustedOracleDemand(encodedDemand);
...
test("oracleArbitration", async () => {
  ...
  const statementHash = await clientUser.stringResult.makeStatement("Statement to arbitrate");
  ...
  const arbitrationResult = await clientUser.arbiters.waitForTrustedOracleArbitration(
