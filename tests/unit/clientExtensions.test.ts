...
describe("Client Extension Tests", () => {
  ...
  test("makeMinimalClient should return only core functionality", () => {
    const minimalClient = makeMinimalClient(
      walletClient,
      testContext.addresses
    );
  ...
  test("makeClient can be extended with custom functionality", () => {
    ...
    const extendedClient = client.extend((baseClient) => ({
  ...
  test("makeClient can chain multiple extensions", () => {
    ...
    const firstExtension = client.extend((baseClient) => ({
      customMethod1: () => "First Extension",
    }));

    const secondExtension = firstExtension.extend((baseClient) => ({
      customMethod2: () => "Second Extension",
    }));

    const thirdExtension = secondExtension.extend((baseClient) => ({
